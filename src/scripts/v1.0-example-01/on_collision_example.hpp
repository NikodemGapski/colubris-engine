...
public:
	...
	void update() {
		...
		parent_transform->rotation = sin(time_value + math::PI / 8.0f) * 90.0f;
	}
	void on_collision(GameObject* other) {
		...
		for(auto pt : points) {
			GameObject* a = new GameObject(DefaultMesh::Circle, {2.0f}, {}, {{0.9f, 0.3f, 0.3f}}, NULL);
			a->get_component<Transform>()->position = pt;
